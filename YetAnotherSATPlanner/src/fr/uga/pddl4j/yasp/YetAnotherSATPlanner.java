...
public class YetAnotherSATPlanner extends AbstractStateSpacePlanner {
    ...
    static final int MAXSTEPS = 100; // Augmenter le nombre maximal d'étapes
    ...
    static final boolean DEBUG = true; // Activer les logs de débogage
        ...
        if (hlb > MAXSTEPS) {
            ...
            while (doSearch && !(steps > stepmax)) {
                ...
                for (List<Integer> clause : sat.currentDimacs) {
                    try {
                        IntStream intStream = clause.stream().mapToInt(Integer::intValue);
                        int[] intArray = intStream.toArray();
                        VecInt vecIntClause = new VecInt(intArray);
                        solver.addClause(vecIntClause);

                    } 
                    catch (ContradictionException e) {
                        System.out.println("Contradiction trouvée (clause)");
                        e.printStackTrace();
                        return null;
                    }
                }
                ...
                try {
                    for (Integer lit : sat.currentGoal) {
                        int[] clauseTab = new int[]{lit}; 
                        VecInt vecIntClause = new VecInt(clauseTab);
                        solver.addClause(vecIntClause);
                    }
                } 
                catch (ContradictionException e) {
                    System.out.println("Contradiction trouvée (but)");
                    e.printStackTrace();
                    return null;
                }
                ...
                try {
                    if (solver.isSatisfiable()) {
                        ...
                        List<Integer> solution = IntStream.of(model).boxed().collect(Collectors.toList());
                        ...
                        sat.next();
                        if (DEBUG) {
                        System.out.println("Pas de solution pour " + steps + " steps.");
                        }
                    }
                } catch (TimeoutException e) {
                    System.out.println("Timeout du solver");
                    e.printStackTrace();
                    return null;
                }
            }
